...
command -v unzip >/dev/null ||
    error 'unzip is required to install imagextract. This can be installed with Homebrew: https://formulae.brew.sh/formula/unzip'
...
if [[ $(uname) != "Darwin" ]]; then
    error 'Mac OS is required to run imagextract'
fi


# Check if the Mac OS version is greater than or equal to 13
if [[ $(sw_vers -productVersion | cut -d . -f 1) -lt 13 ]]; then
    error 'Mac OS 13 or greater is required to run imagextract since it depends on Apple''s updated Vision APIs. Please upgrade your Mac and try again.'
fi
...
curl --fail --location --progress-bar --output "$exe.zip" "$download_uri" ||
    error "Failed to download imagextract from \"$download_uri\""
...
chmod +x "$exe" ||
    error 'Failed to set permissions on imagextract executable'
